...
2. 用官方 `codex app-server daemon stop` 停止托管后台，对剩余的当前用户 Codex 二进制进程发送 SIGTERM；若进程不退出或自动重连，停止操作并报告。
3. 执行 `codex update`，除非指定 `--skip-update`。
4. 把唯一目标 `models_cache.json` 移入一个新的私有备份目录，不覆盖以往备份。
5. 分别获取内置目录、刷新命令结果，启动新版托管后台，然后检查实际磁盘缓存。
6. 单独报告目标模型是否存在、可见性、推理强度，以及缓存版本。未确认进入可见缓存时退出码为 3；其他错误非零，完成检查为 0。
...
如需回滚缓存，先退出相关客户端并停止后台，再把脚本输出目录里的 `models_cache.json` 复制回原配置目录。脚本失败且没有生成替代缓存时会自动恢复旧缓存；不会覆盖失败过程中已生成的新缓存。后台若保持停止，可运行 `codex app-server daemon start`。
